...
# ============================================================================
# CONFIGURAZIONE GLOBALE (MODIFICATA)
# ============================================================================

# BASE_DIR non è più definito qui. Verrà impostato in main() dal menu.

# Percorso LOGS è ora fisso
...
# ============================================================================
# SETUP LOGGING (MODIFICATO)
# ============================================================================

def setup_logging():
    """Configura logging per l'intera pipeline."""
    os.makedirs(LOG_DIR_ROOT, exist_ok=True) # Usa LOG_DIR_ROOT
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    log_filename = LOG_DIR_ROOT / f'unified_pipeline_{timestamp}.log' # Usa LOG_DIR_ROOT
...
def ask_continue_to_cropping():
    ...
    while True:
        print("\n" + "─"*70)
        # Il prossimo script si chiama 'step2_croppedmosaico.py'
...
# ============================================================================
# MAIN: ESECUZIONE PIPELINE (MODIFICATO)
# ============================================================================
...
def main_step2(INPUT_HUBBLE, INPUT_OBSERVATORY, OUTPUT_HUBBLE, OUTPUT_OBSERVATORY, BASE_DIR, logger):
    ...
    # --- ECCO LA CORREZIONE ---
    # 'create_common_wCS_frame' è diventato 'create_common_wcs_frame'
...
def main():
    ...
    logger.info(f"TARGET DIRECTORY: {BASE_DIR}") # Logga la cartella scelta
    logger.info("=" * 60)
    ...
    # INPUT STEP 1
    INPUT_OSSERVATORIO = BASE_DIR / '1_originarie' / 'local_raw'  # MODIFICATO
    INPUT_LITH = BASE_DIR / '1_originarie' / 'img_lights'     # MODIFICATO

    # OUTPUT STEP 1 (che diventa INPUT STEP 2)
    OUTPUT_OSSERVATORIO_WCS = BASE_DIR / '2_wcs' / 'osservatorio' # MODIFICATO
    OUTPUT_LITH_WCS = BASE_DIR / '2_wcs' / 'hubble'           # MODIFICATO
    ...
    if step1_success and step2_success:
        ...
        # === INSERIMENTO NUOVO MENU ===
        if ask_continue_to_cropping():
            try:
                # Assumiamo che step2_croppedmosaico.py sia nella stessa cartella
                script_dir = Path(__file__).parent
                next_script = script_dir / 'step2_croppedmosaico.py'

                if next_script.exists():
                    print(f"\n🚀 Avvio {next_script.name}...")
