"""
STEP 6: ESTRAZIONE PATCHES (MIGLIORATO - RISOLUZIONE NATIVA)
...
MENU: Scegli se usare immagini CROPPED (uniformi) o REGISTERED (dimensioni originali)
"""
...
# MENU: Input selezionabile dall'utente
# Opzione 1: Immagini CROPPED (tutte stesse dimensioni - CONSIGLIATO)
INPUT_CROPPED_HUBBLE = Path(BASE_DIR) / '4_cropped' / 'hubble'
INPUT_CROPPED_OBSERVATORY = Path(BASE_DIR) / '4_cropped' / 'observatory'

# Opzione 2: Immagini REGISTERED (dimensioni originali)
...
# Variabili che verranno impostate dal menu
INPUT_HUBBLE = None
...
# ============================================================================
# MENU SELEZIONE TIPO INPUT
# ============================================================================

def select_input_type():
    """
    Mostra un menu per scegliere tra immagini CROPPED o REGISTERED.
    Imposta le variabili globali INPUT_HUBBLE, INPUT_OBSERVATORY, OUTPUT_DIR.
    """
    global INPUT_HUBBLE, INPUT_OBSERVATORY, OUTPUT_DIR
    ...
    print(f"   ✓ Ottimale per estrazione patches uniformi")
    print(f"   ✓ Nessun bordo con dimensioni diverse")
    print(f"   📁 Input: {INPUT_CROPPED_HUBBLE.parent}")
    
    print(f"\n2️⃣  IMMAGINI REGISTERED")
    print(f"   • Mantiene le dimensioni originali di ogni immagine")
    print(f"   • Può avere dimensioni variabili tra immagini")
    print(f"   • Utile se vuoi mantenere FOV completo")
    ...
    if not INPUT_HUBBLE.exists():
        print(f"   ⚠️  ATTENZIONE: Directory Hubble non trovata: {INPUT_HUBBLE}")
        print(f"   Assicurati di aver eseguito gli step precedenti!")
    else:
        hubble_files = list(INPUT_HUBBLE.glob('*.fits')) + list(INPUT_HUBBLE.glob('*.fit'))
        print(f"   ✓ Hubble: {len(hubble_files)} file trovati")
    
    if not INPUT_OBSERVATORY.exists():
        print(f"   ⚠️  ATTENZIONE: Directory Observatory non trovata: {INPUT_OBSERVATORY}")
        print(f"   Assicurati di aver eseguito gli step precedenti!")
    else:
        obs_files = list(INPUT_OBSERVATORY.glob('*.fits')) + list(INPUT_OBSERVATORY.glob('*.fit'))
        print(f"   ✓ Observatory: {len(obs_files)} file trovati")
    
    print(f"\n📂 Output patches verranno salvate in: {OUTPUT_DIR}")
...
def print_pipeline_summary(logger):
    ...
    # Mostra sia cropped che registered
    analyze_directory(INPUT_CROPPED_HUBBLE, "Step 3: Cropped (da Step 3)", recursive=False, scale_key='NATIVESC', logger=logger)
    analyze_directory(INPUT_REGISTERED_HUBBLE, "Step 3: Registered (da Step 2)", recursive=False, scale_key='NATIVESC', logger=logger)
    analyze_directory(OUTPUT_HUBBLE_PATCHES, "Step 6: Patches Finali (questo step)", recursive=False, scale_key='PIXSCALE', logger=logger)
    ...
    # Mostra sia cropped che registered
    analyze_directory(INPUT_CROPPED_OBSERVATORY, "Step 3: Cropped (da Step 3)", recursive=False, scale_key='NATIVESC', logger=logger)
    analyze_directory(INPUT_REGISTERED_OBSERVATORY, "Step 3: Registered (da Step 2)", recursive=False, scale_key='NATIVESC', logger=logger)
    analyze_directory(OUTPUT_OBS_PATCHES, "Step 6: Patches Finali (questo step)", recursive=False, scale_key='PIXSCALE', logger=logger)
        ...
        with fits.open(image_path) as hdul:
            ...
            for y in range(0, ny - patch_size + 1, step):
                for x in range(0, nx - patch_size + 1, step):
                    ...
                    patch_header['VALIDPCT'] = (valid_percent, 'Valid pixels (%)')
...
def main():
    ...
    # MENU: Seleziona tipo di input
    source_type = select_input_type()
    
    logger = setup_logging()
    logger.info("=" * 60)
    logger.info(f"ESTRAZIONE PATCHES CON RISOLUZIONE NATIVA")
    logger.info(f"Tipo input: {source_type.upper()}")
    logger.info("=" * 60)
    
    print("\n" + "✂️ "*35)
    print(f"STEP 6: ESTRAZIONE PATCHES (RISOLUZIONE NATIVA)".center(70))
    print("✂️ "*35)
