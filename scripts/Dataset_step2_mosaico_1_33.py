...
def crop_all_images_for_target(base_dir):
    ...
    for filepath in tqdm(all_files, desc="Ritaglio", unit="file"):
        ...
        if crop_image(filepath, output_dirs[category] / filepath.name, min_height, min_width):
            success_count += 1
...
def create_mosaic_for_target(base_dir):
    ...
    try:
        with fits.open(all_files[0]) as hdul:
            ...
            shape = hdul[0].data.shape if len(hdul[0].data.shape) == 2 else hdul[0].data[0].shape
    except Exception as e:
        print(f"❌ Err lettura primo file: {e}")
        return False
    ...
    final_path = mosaic_out / 'final_mosaic.fits'
...
def main():
    ...
    # Se riceve un argomento, usa quello come path assoluto
    if len(sys.argv) > 1:
