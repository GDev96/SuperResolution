...
def save_png_preview(lr, sr, path):
    """Salva un confronto visivo rapido in PNG: LR | SR"""
    # Ridimensiona LR alle dimensioni di SR per affiancarle (Nearest Neighbor per vedere i pixel)
...
def run(args):
    ...
    try:
        ...
        print(f"❌ Errore apertura FITS: {e}")
        return
    ...
    # Calcolo dimensioni target basato sulla scala
    ...
    try:
        ...
        # Gestione se il file contiene l'intero stato o solo i pesi
    ...
    # Ripristina il range dinamico originale dell'immagine di input
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', required=True, help="Path al file .pth (es. outputs/.../last_model_light.pth)")
    parser.add_argument('--input', required=True, help="Path immagine FITS Low-Res (Input)")
    parser.add_argument('--output', required=True, help="Path dove salvare l'output FITS")
    
    # Parametro Scale aggiornato al default del training (4.0x)
    parser.add_argument('--scale', type=float, default=4.0, help="Fattore di scala (default: 4.0 per training Light 128->512)")
