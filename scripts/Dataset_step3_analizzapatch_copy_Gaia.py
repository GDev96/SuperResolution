...
def main():
    ...
    for target_dir in ROOT_DATA_DIR.iterdir():
        ...
        # Seleziona 3 esempi sparsi (inizio, metà, fine)
        samples_coords = []
        if total_pairs > 0:
            indices = np.linspace(0, total_pairs - 1, 3, dtype=int)
            for idx in indices:
                t = extraction_tasks[idx]
                samples_coords.append(f"#{t['pair_id']:05d}: RA={t['ra']:.5f}, DEC={t['dec']:.5f}")
        ...
        print(f"   ------------------------------------------------")
        print(f"   📍 ESEMPI COORDINATE (Campioni):")
        if samples_coords:
            for s in samples_coords:
                print(f"      {s}")
        else:
