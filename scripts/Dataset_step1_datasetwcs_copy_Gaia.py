...
def print_registration_report():
    """
    Genera e stampa un report tabellare delle immagini registrate in 3_registered_native
    con dettagli su dimensioni, coordinate centrali e pixel scale.
    """
    import pandas as pd
    import numpy as np
    from astropy.wcs import WCS
    ...
    print("\n" + "="*120)
    print("📊 REPORT FINALE REGISTRAZIONE WCS".center(120))
    print("="*120)
    
    results = []
    
    # Assicurati che ROOT_DATA_DIR sia accessibile (o ridefiniscilo qui se locale al main)
    ...
    for target_dir in ROOT_DATA_DIR.iterdir():
        ...
        reg_base = target_dir / '3_registered_native'
        
        paths = [
            (reg_base / 'hubble', 'Hubble (HR)'),
            (reg_base / 'observatory', 'Observatory (LR)')
        ]
        
        for p_dir, origin in paths:
            if p_dir.exists():
                for f in sorted(p_dir.glob("*.fits")):
                    try:
                        with fits.open(f) as hdul:
                            ...
                            # Info WCS
                            wcs = WCS(header)
                            center = wcs.pixel_to_world(w/2, h/2)
                            scales = proj_plane_pixel_scales(wcs)
                            px_scale = np.mean(scales) * 3600 # arcsec
                            ...
                            results.append({
                                ...
                                'Dim': f"{w}x{h}",
                                'RA Centro': f"{center.ra.deg:.5f}°",
                                'DEC Centro': f"{center.dec.deg:.5f}°",
                                'Scale': f"{px_scale:.3f}\"/px"
                            })
                    except Exception as e:
                        pass
    ...
    if results:
        ...
        df = df.sort_values(['Target', 'Origine'])
        print(df.to_string(index=False))
    else:
        print("⚠️ Nessuna immagine trovata in 3_registered_native.")
